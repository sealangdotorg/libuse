...
Boolean Type::createBoolean( const std::string& value )
{
    return Boolean::fromString( value );
}

Boolean Type::createBoolean( const u1 value )
{
...
Boolean Boolean::fromString( const std::string& value )
{
    auto v = libstdhl::String::value( value );
    ...
    throw std::domain_error( "invalid '" + value + "' Boolean literal" );
    return Boolean();
}

u1 Boolean::value( void ) const
{
    assert( defined() );
    assert( trivial() );
    return m_data.value;
