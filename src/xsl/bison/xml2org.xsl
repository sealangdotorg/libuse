...
<!--
    transform bison grammar xsl (xml) representation to a org-mode content page
    with dokuwiki internal include commands to reference a description page

    derived from xml2text.xsl - transform Bison XML Report into plain text.
 -->
...
<xsl:template match="item">
  ...
  <xsl:param name="next-rule-number"
             select="following-sibling::item[1]/@rule-number"/>
  <xsl:apply-templates
  ...
  >
   ...
   />
    <xsl:with-param
      name="next-lhs"
      select="key('bison:ruleByNumber', $next-rule-number)/lhs[text()]"
   />
...
<xsl:template match="rule">
  ...
  <xsl:choose>
    <xsl:when test="$itemset != 'true' and $prev-lhs = lhs[text()]">
      <xsl:call-template name="lpad">
        ...
        <xsl:with-param name="pad" select="number(string-length(lhs[text()])) + 4"/>
      </xsl:call-template>
    </xsl:when>
  ...
  <!-- end of rule -->
  <xsl:if test="not($next-lhs = lhs[text()])">
    <xsl:text>#+end_src&#10;</xsl:text>
    <xsl:text>&#10;</xsl:text>
