...
TEST_P( cpp_Args, param )
{
    ...
    for( auto param_opt : param.options )
    {
        ...
        function< void( const char* ) > hook =
        [ param_opt, &cnt_options_chr, &cnt_options_str]( const char* arg )
        {
            //printf("%s: '%c' \"%s\" {%u}\n", TEST_NAME, param_opt.chr, param_opt.str, param_opt.req);
...
, ::testing::Values
  ...
  , Param
    { { "very_long_file_name", "short_file" }
    , 2
    }
  , Param
